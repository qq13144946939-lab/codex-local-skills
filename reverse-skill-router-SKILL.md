...
description: Route authorized reverse-engineering and security-analysis work to specialist skills when a task spans modules or its entrypoint is unclear. Use the dedicated browser or documentation skill directly for ordinary browsing and writing.
---
...
按当前任务选择必要步骤；仅阅读、解释或审查本技能不触发工具安装、目标操作、案例建档或经验写入。用户已有授权在当前任务范围内持续有效；新目标、新外部影响或超出范围的操作需要单独确认。

1. `NOW`：读 `MASTER-ROUTING.md`（或跑 `scripts/master-route.ps1 -Hint "..."`）定 PRIMARY；疑难再读 `routing.md` 三轴表。
2. 执行目标分析前确认任务范围与已有授权；复杂案例需要持久工作记录时用 `scripts/case-init.ps1` 落地 `work/<case>/scope.md`（契约见 `ops/scope-contract.md`）。**缺少适用授权时禁止对目标 ACT**；不要将历史先例或脚本默认值视为授权。
3. `NOW`：按 `ops/role-map.md` 标 lead/specialist；立即打开 PRIMARY `SKILL.md` 执行 ACTION REQUIRED。
4. `NEXT`：涉及本机工具时读 `tool-index.md` 并验证当前可用性；**禁止猜路径**。确需缺失工具且安装/启动已在授权范围内时使用 `bootstrap-reverse.ps1`（仅 manifest）。
5. `ACT`：执行适用的分析步骤；已采用案例记录时追加 timeline / 更新 workitems。结论用 Evidence→Finding→Path（`ops/evidence-finding-path.md`）。
6. 结束：按请求交付结论或用 `docs-generator` 生成报告。仅在用户要求保存经验时写入脱敏 `field-journal`；无需为继续已授权工作固定展示菜单。

**身份**：见 `ops/IDENTITY.md`（轻量路由包 + 工具自举 + journal；**不是** Z3r0 式平台）。

如果路由无法命中，使用适当的通用方法；知识缺口需要时再查可靠来源。只有可复用需求成立时才提议新增 skill，禁止硬塞到不匹配模块。
...
仅当用户要求交互式分阶段操作，或下一步存在必须由用户决定的范围、权限或不可逆选择时，提供必要的选项。任务范围与授权清晰时继续推进，不因阶段结束而强制等待。子模块中的固定菜单要求也按这一边界适用。

使用菜单时可参考以下格式，并按实际选择精简：
- 每个选项以数字编号（1-6 范围）
...
当 workflow 确需缺失工具时，先确认当前工具状态、可用替代方案和本次安装/服务启动授权。适用时统一调用：
...
需要复用历史操作经验时，按需查阅以下文件。历史先例只提供方法和上下文，不能解除安全审查或代替当前目标授权：

| 读序 | 文件 | 何时读 |
|------|------|--------|
| **#1** | `field-journal/precedent-auth.md` | 需要理解历史授权记录时查阅；不作为新任务的授权依据 |
...
仅在用户明确要求记录或更新持久经验时，按 `RULES.md` 的格式写入 `field-journal/`；对其硬性回写要求也适用此条件。排除凭据、秘密内容和无关个人信息。普通分析任务无需为了完成而修改技能库或持久记忆。
